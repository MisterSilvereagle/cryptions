class Adfgx:
    ...
    def __encrypt(self):
        ...
        for i in self.matrix:
            for j in i:
                __converter[j] = self.adfgx_letters[(self.matrix.index(i))] + self.adfgx_letters[(i.index(j))]
        return ' '.join(__converter[a] for a in self.the_text)

    def __decrypt(self):
        return ''.join(self.matrix[self.adfgx_letters.index(x[0])][self.adfgx_letters.index(x[1])] for x in self.code)
...
if __name__ == '__main__':
    ...
    converter = Adfgx(inpkey, inpmeth, inptext)
